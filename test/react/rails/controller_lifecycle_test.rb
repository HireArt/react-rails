...
# calls to `react_rails_component`
class DummyHelperImplementation
  ...
  def react_rails_component(*args)
    @events << :react_rails_component
  end
end
...
class ControllerLifecycleTest < ActionDispatch::IntegrationTest
  ...
  test "it creates a helper object and puts it in the request env" do
    get '/pages/1'
    helper_obj = controller.__react_rails_component_helper
  ...
  test "it calls setup and teardown methods" do
    ...
    helper_obj = controller.__react_rails_component_helper
    lifecycle_steps = ["123", :react_rails_component, :teardown]
    assert_equal(lifecycle_steps, helper_obj.events)
  end

  test "there's a new helper object for every request" do
    get '/pages/1'
    first_helper = controller.__react_rails_component_helper
    get '/pages/1'
    second_helper = controller.__react_rails_component_helper
